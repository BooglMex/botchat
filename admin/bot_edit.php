...
if(isset($_POST["form_submit"])){
	...
	if(count($error)) $msgtext = '<div class="alert-danger">'.implode('<br>',$error).'</div>';
		...
		mysql_query("UPDATE bots SET bot_name='$form_name', bot_admin=$form_parent WHERE bot_id=$id", $link);
		$msgtext .= '<div class="alert-success">Успешно обновлено!</div>';
		
		$R = mysql_fetch_array( mysql_query("SELECT * FROM bots WHERE bot_id=$id") );
	}
}
?>
<!DOCTYPE html>
            ...
            <div class="container-fluid">
                <div class="row pt-2 pb-2">
                    <div class="col-md-8 offset-md-2">
                        <h4 class="page-title">Изменение бота</h4>
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="/">Админ-панель</a></li>
                            <li class="breadcrumb-item"><a href="bots.php">Боты</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Изменение бота</li>
                        </ol>
                    </div>
                </div>
                                ...
                                <form id="personal-info" method="POST" enctype="multipart/form-data">
								   ...
								   ?>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-12">
                                            <input type="text" name="form_name" value="<?php echo htmlchars($R["bot_name"]); ?>" class="form-control" required>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div class="col-md-12">
                                            <select class="form-control" name="form_parent" required>
                                                <option value="0" disabled>Выберите админа</option>
                                                <?php
                                                $Qget = mysql_query("SELECT * FROM clients ORDER BY cl_fio");
                                                if(mysql_num_rows($Qget)){
                                                    while($Rget = mysql_fetch_array($Qget)){
                                                        $selected = ($Rget["cl_id"] == $R["bot_admin"]) ? "selected" : "";
