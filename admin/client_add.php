...
if(isset($_POST["form_submit"])){
	...
	if(filter_var($form_email, FILTER_VALIDATE_EMAIL) === false) $error[] = "Укажите E-mail в правильном формате";
		...
		$Qcheck = mysql_query("SELECT cl_email FROM clients WHERE cl_email='$form_email'");
		if(mysql_num_rows($Qcheck)) $error[] = "Данный E-mail уже существует. Пожалуйста укажите другую почту.";
	}
	if(!strlen($form_pass)) $error[] = "Укажите пароль";
	elseif($form_pass != $form_pass_repeat) $error[] = "Укажите пароль подтверждения правильно";
	
	if(count($error)) $msgtext = '<div class="alert-danger">'.implode('<br>',$error).'</div>';
	else{
		$form_pass = strrev(md5($form_pass)); // Шифровка пароля
		
		mysql_query("INSERT INTO clients(cl_fio, cl_email, cl_pass) VALUES('$form_name', '$form_email', '$form_pass')", $link);
		if(mysql_insert_id()){
			$id = mysql_insert_id();
            ...
            <div class="container-fluid">
                <div class="row pt-2 pb-2">
                    <div class="col-md-8 offset-md-2">
                        <h4 class="page-title">Добавление клиента</h4>
                        <ol class="breadcrumb">
                                ...
                                <form id="personal-info" method="POST" enctype="multipart/form-data">
                                   <?php echo $msgtext; ?>
									...
									<div class="form-group">
                                        ...
                                        <div class="col-sm-12">
                                            <input type="text" name="form_email" value="<?php echo htmlchars($_POST["form_email"]); ?>" class="form-control" required>
                                        </div>
                                    </div>
									<div class="form-group">
                                        <label class="col-sm-12 col-form-label">Пароль</label>
                                        <div class="col-sm-12">
                                            <input type="password" name="form_pass" value="<?php echo htmlchars($_POST["form_pass"]); ?>" class="form-control" required>
                                        </div>
                                    </div>
									<div class="form-group">
                                        <label class="col-sm-12 col-form-label">Повторите пароль</label>
                                        <div class="col-sm-12">
                                            <input type="password" name="form_pass_repeat" value="<?php echo htmlchars($_POST["form_pass_repeat"]); ?>" class="form-control" required>
                                        </div>
                                    </div>
