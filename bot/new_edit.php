...
$id = $_GET["id"];
/* $Q = mysql_query("SELECT * FROM news WHERE new_id=$id");
...
if(isset($_POST["form_submit"]))
{
   ...
   // Принятие информации
    ...
    $form_descr_mini = clear_string($_POST["form_descr_mini"]);
    ...
    {
        mysql_query("UPDATE news SET new_name='$form_name', new_descr=$form_descr, new_descr_mini=$form_descr_mini WHERE new_id=$id", $link);

        $_SESSION['msgtext'] .= '<div class="alert-success">Новость успешно изменена!</div>';

        if(is_uploaded_file($_FILES['form_image']['tmp_name']))
        {
            $imgPrefiks = "";
            $uploaddir = 'images/news/'; // Папка для загрузки
            $queryAddImg1 = "UPDATE news SET new_image='";
            $queryAddImg2 = "' WHERE new_id=$id";
            ...
            else @unlink('images/news/'.$R['new_image']);
		...
		<div class="content-wrapper">
			<div class="container-fluid">
				<div class="row pt-2 pb-2">
					<div class="col-12">
						<h4 class="page-title">Изменение новости</h4>
						<ol class="breadcrumb">
					...
					<div class="col-md-8 offset-md-2">
						<div class="card">
							<div class="card-body">
								...
								{
									$q_pr = mysql_query("SELECT * FROM news WHERE new_id=$id");
									if(mysql_num_rows($q_pr) == 1)
									{
										...
										<form id="personal-info" method="POST" enctype="multipart/form-data">
											...
											<div class="form-group">
												<label class="col-md-12 col-form-label">Наименование новости</label>
												<div class="col-sm-12">
													<input type="text" name="form_name" value="'.htmlchars($r_pr['new_name']).'" class="form-control" required>
												</div>
											</div>';
											
											if(strlen($r_pr["new_image"]) > 0 && file_exists("images/news/".$r_pr["new_image"]) && $r_pr["new_image"] != "no_image.png")
											{
												$img_path = 'images/news/'.$r_pr["new_image"];
												$max_width = 160;
												$max_height = 160;
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-12">
                                                    <textarea class="form-control" rows="4" name="form_descr_mini" required>'.htmlchars($r_pr['new_descr_mini']).'</textarea>
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-group">
                                                <div class="col-lg-12">
                                                    <div class="card">
                                                        ...
                                                        <div class="card-body">
                                                            <textarea class="summerNoteEditor" id="summerNoteEditor1" name="form_descr">'.$r_pr["new_descr"].'</textarea>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div>
